...
#define BMDebugStr(str) (str ? [NSString stringWithFormat:@"%@", str] : @"-")
#define BMDebugBool(expr) ((expr) ? @"Yes" : @"No")
#define BMDebugDesc(expr) ((expr != nil) ? [expr description] : @"-")

@interface BMDebugSystemInfo ()<UISearchResultsUpdating, UISearchControllerDelegate>
...
@implementation BMDebugSystemInfo
...
- (void)initSystemInfo
{
    ...
    [rowsData addObjectsFromArray:@[
                                    @{ @"Name" : BMDebugStr(applicationName) },
                                    @{ @"Version" : BMDebugStr(version) },
                                    @{ @"Build" : BMDebugStr(build) },
                                    ...
                                    @{ @"Debug" : BMDebugBool([self isDebug]) },
    ...
    if (urlSchemes.count > 1) {
        for (int i = 0; i < urlSchemes.count; i++) {
            [rowsData addObject:@{ @"Url Schemes" : BMDebugStr([urlSchemes objectAtIndex:i]) }];
        }
    } else {
        [rowsData addObject:@{ @"Url Scheme" : BMDebugStr(urlSchemes.count > 0 ? [urlSchemes objectAtIndex:0] : nil) }];
    }
    ...
    sectionData = @{
                    ...
                    @"rows": @[
                            ...
                            @{ @"Low Power Mode" : BMDebugBool([[NSProcessInfo processInfo] isLowPowerModeEnabled]) }
                            ]
                    };
    [self.systemInfo addObject:sectionData];
    ...
    sectionData = @{
                    ...
                    @"rows": @[
                            ...
                            @{ @"UUID" : BMDebugStr(self.identifierUUID) },
                            @{ @"Jailbroken" : BMDebugBool(self.isJailbreak) },
                            ]
                    };
    [self.systemInfo addObject:sectionData];
    ...
    sectionData = @{
                    @"title": @"Network",
                    @"rows": @[
                            @{ @"MAC Address" : BMDebugDesc(self.macAddress) },
                            @{ @"SSID" : BMDebugDesc(self.SSID) },
                            @{ @"BSSDID" : BMDebugDesc(self.BSSID) },
    ...
    sectionData = @{
                    ...
                    @"rows": @[
                            ...
                            @{ @"Carrier Name" : BMDebugDesc([info.subscriberCellularProvider.carrierName capitalizedString]) },
                            @{ @"Data Connection": BMDebugDesc([info.currentRadioAccessTechnology stringByReplacingOccurrencesOfString:@"CTRadioAccessTechnology" withString:@""]) },
                            @{ @"Country Code" : BMDebugDesc(info.subscriberCellularProvider.mobileCountryCode) },
                            @{ @"Network Code" : BMDebugDesc(info.subscriberCellularProvider.mobileNetworkCode) },
                            @{ @"ISO Country Code" : BMDebugDesc(info.subscriberCellularProvider.isoCountryCode) },
                            @{ @"VoIP Enabled" : BMDebugBool(info.subscriberCellularProvider.allowsVOIP) }
                            ]
                    };
    [self.systemInfo addObject:sectionData];
}
