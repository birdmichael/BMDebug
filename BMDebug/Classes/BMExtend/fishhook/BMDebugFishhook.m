...
// 替换NSLog
void bmDebug_NSLog(NSString *format, ...)
{
    va_list args;
    if (format) {
        ...
        orig_NSLog(@"%@", message);
        [BMDebugFishhook logMessage:message];
    }
}

// 替换NSLogv
void bmDebug_NSLogv(NSString *format, va_list args)
{
    va_list copy_args;
...
#pragma mark - BMDebugFishhook

static NSMutableArray *_allLogs;
static NSUInteger _logLimit;


@implementation BMDebugFishhook
...
+ (void)load
{
    ...
    rcd_rebind_symbols((struct rcd_rebinding[2]){
        {"NSLog", bmDebug_NSLog, (void *)&orig_NSLog},
        {"NSLogv", bmDebug_NSLogv, (void *)&orig_NSLogv}
    }, 2);
}
...
+ (void)logMessage:(NSString *)message
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        logPath = [[logPath stringByAppendingPathComponent:@"BMDebug"] stringByAppendingPathComponent:@"NSLog"];
        ...
        // 文件写队列，串行
        _logFileQueue = dispatch_queue_create("com.ocphp.BMDebugNSLog", DISPATCH_QUEUE_SERIAL);
...
+ (void)mergeLogFiles:(NSString *)logPath
{
    ...
    for (NSString *fileName in fileNames) {
        // 只保留最近7天的日志
        if (fileName.length == 18 && [fileName hasPrefix:@"BMLog-"]) {
...
+ (NSUInteger)logLimit{
    ...
    return _logLimit!=0 ? _logLimit : 100;
}
